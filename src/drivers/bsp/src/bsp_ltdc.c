...
/* 硬件自定义参数 */
#define LTDC_PIXEL_FORMAT                   LCD_PIXEL_FORMAT_RGB565
#define LTDC_LAYER_1                        0
#define LTDC_LAYER_2                        1
...
//根据不同的颜色格式,定义帧缓存数组
#if (LTDC_PIXEL_FORMAT==LCD_PIXEL_FORMAT_ARGB8888)||(LTDC_PIXEL_FORMAT==LCD_PIXEL_FORMAT_RGB888)
...
{ 
	...
	DMA2D->OPFCCR=LTDC_PIXEL_FORMAT;	//设置颜色格式
...
void LTDC_Color_Fill(u16 sx,u16 sy,u16 ex,u16 ey,u16 *color)
{
	...
	DMA2D->FGPFCCR=LTDC_PIXEL_FORMAT;	//设置颜色格式
...
//设置LCD显示方向
//dir:0,竖屏；1,横屏
static void LTDC_Display_Dir(u8 dir)
{
...
static void LCD_Config(void)
{
	...
	lcdltdc.vfp=12;				    //垂直前廊
	LTDC_Clk_Set(396,2,RCC_PLLSAIDIVR_4);//设置像素时钟  49.5MHz(45 ~ 57MHz)
...
void LTDC_Layer_Parameter_Config(u8 layerx,u32 bufaddr,u8 pixformat,u8 alpha,u8 alpha0,u8 bfac1,u8 bfac2,u32 bkcolor)
{
	...
	pLayerCfg.BlendingFactor1=LTDC_BLENDING_FACTOR1_CA;//(u32)bfac1<<8;    //设置层混合系数
	pLayerCfg.BlendingFactor2=LTDC_BLENDING_FACTOR2_CA;//(u32)bfac2<<8;	//设置层混合系数
...
void bsp_LTDC_Init(void)
{
	...
	LTDC_Select_Layer(LTDC_LAYER_1); 	//选择第1层
	LCD_BackLight(1);
