...
//#define WAIT_BUSY()	do{uint8_t kk; for (kk = 0; kk < 40; kk++);  while( NAND_RB == 0 );}while(0);
#define WAIT_BUSY()	NAND_DelayUS(50);
//#define WAIT_BUSY()	    NAND_WaitRB(0);
...
void NAND_DelayUS(vu32 i)
{
    bsp_DelayUS(i);
}
void NAND_DelayMS(vu32 i)
{
    bsp_DelayMS(i);
}
//等待RB信号为某个电平
...
u32 NAND_ReadID(void)
{
    u8 deviceid[5];
    u32 id;
    NAND_CMD_AREA = 0x90; //发送读取ID命令
    NAND_ADDR_AREA = 0X00;
    //ID一共有5个字节
    deviceid[0] = *(vu8 *)Bank_NAND_ADDR;
    deviceid[1] = *(vu8 *)Bank_NAND_ADDR;
    deviceid[2] = *(vu8 *)Bank_NAND_ADDR;
    deviceid[3] = *(vu8 *)Bank_NAND_ADDR;
    deviceid[4] = *(vu8 *)Bank_NAND_ADDR;
...
*********************************************************************************************************
*/
static uint8_t FMC_NAND_PageCopyBackEx(uint32_t _ulSrcPageNo,
    uint32_t _ulTarPageNo,
    uint8_t *_pBuf, 
    uint16_t _usOffset,
    uint16_t _usSize)
{
    ...
    for (n = 0; n < 10; n++)
    {
        ...
        for (i = 0; i < NAND_BLOCK_SIZE; i++)
        {
            if (i == usOffsetPageNo)
            {
                ...
                if (FMC_NAND_PageCopyBackEx(ulSrcBlock * NAND_BLOCK_SIZE + i,
                    usNewBlock * NAND_BLOCK_SIZE + i,
                    _pWriteBuf,
                    _usOffset,
                    _usSize) == NAND_FAIL)
                {
...
void NAND_ScanAllBadBlock(void)
{
    ...
    for (i = 0; i < NAND_BLOCK_COUNT; i++)
    {
        ...
        {
            printf_safe("Scan Block %d (%d%%), Ok\r\n", i, i * 100 / NAND_BLOCK_COUNT);
        }
        else
        {
            printf_safe("Scan Block %d (%d%%), Err\r\n", i, i * 100 / NAND_BLOCK_COUNT);
            NAND_MarkBadBlock(i);
        }
    }
}
...
void NAND_DispBadBlockInfo(void)
{
    ...
    uint16_t bad_no[128];
    	...
    	//FMC_NAND_HALInit();	/* 初始化FSMC */

    id = NAND_ReadID();

    printf_safe("NAND Flash ID = 0x%04X, Type = ", id);
    if (id == HY27UF081G2A)
    {
        printf_safe("HY27UF081G2A\r\n  1024 Blocks, 64 pages per block, 2048 + 64 bytes per page\r\n");
    }
    else if (id == K9F1G08U0A)
    {
        printf_safe("K9F1G08U0A\r\n  1024 Blocks, 64 pages per block, 2048 + 64 bytes per page\r\n");
    }
    else if (id == K9F1G08U0B)
    {
        printf_safe("K9F1G08U0B\r\n  1024 Blocks, 64 pages per block, 2048 + 64 bytes per page\r\n");
    }
    else if (id == H27U4G8F2DTR)
    {
        printf_safe("H27U4G8F2DTR\r\n  4096 Blocks, 64 pages per block, 2048 + 64 bytes per page\r\n");
    }
    else if (id == MT29F4G08ABADA)
    {
        printf_safe("MT29F4G08ABADA\r\n  4096 Blocks, 64 pages per block, 2048 + 64 bytes per page\r\n");
    }
    else
    {
        printf_safe("unkonow\r\n");
    ...
    printf_safe("Block Info : 0 is OK, * is Bad, - is Used\r\n");
    n = 0;	/* 坏块统计 */
    used = 0; /* 已用块统计 */
    for (i = 0; i < NAND_BLOCK_COUNT; i++)
    {
    ...
    if (id == H27U4G8F2DTR)
    {
        printf_safe("Bad Block Count = %d  ( < 80 is OK), Used = %d \r\n", n, used);
    }
    else
    {
        printf_safe("Bad Block Count = %d\r\n", n);
    }
...
void NAND_DispPhyPageData(uint32_t _uiPhyPageNo)
{
    ...
    {
        printf_safe("FMC_NAND_ReadPage Failed() \r\n");
        return;
    }

    printf_safe("Block = %d, Page = %d\r\n", ulBlockNo, usOffsetPageNo);
    ...
    for (n = 0; n < 4; n++)
    {
        for (i = 0; i < 512; i++)
        {
            printf_safe(" %02X", s_ucTempBuf[i + n * 512]);
    ...
    for (i = 0; i < 64; i++)
    {
        printf_safe(" %02X", s_ucTempBuf[i + 2048]);
...
void NAND_DispLogicPageData(uint32_t _uiLogicPageNo)
{
    ...
    printf_safe("LogicBlock = %d, PhyBlock = %d\r\n", _uiLogicPageNo, usPBN);
...
void NAND_DispParamPage(void)
{
    ...
    printf_safe("\r\n");
    printf_safe("Read Parameter Page Data :\r\n");
    printf_safe("Sign = %c%c%c%c\r\n", tPage.Sign[0], tPage.Sign[1], tPage.Sign[2], tPage.Sign[3]);
    printf_safe("Revision = %04X\r\n", tPage.Revision);
    printf_safe("Features = %04X\r\n", tPage.Features);
    printf_safe("OptionalCommands = %04X\r\n", tPage.OptionalCommands);

    	/* Manufacturer information block */
    memcpy(buf, tPage.Manufacturer, 12);
    buf[12] = 0;
    printf_safe("Manufacturer = %s\r\n", buf);	/* 制造商 */

    memcpy(buf, tPage.Model, 20);
    buf[20] = 0;
    printf_safe("Model = %s\r\n", buf);	/* 型号 */

    printf_safe("JEDEC_ID = %02X\r\n", tPage.JEDEC_ID);	/* AD */
    printf_safe("DateCode = %04X\r\n", tPage.DateCode);
