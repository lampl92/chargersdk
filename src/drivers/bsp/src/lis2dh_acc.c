...
*                      lis2de12 adapter board.
* HISTORY:
* Date        | Modification                                | Author
* 30/08/2016  | Initial Revision                            | Fabio Tota
...
static void ReadMultiReg(const char *cmd);
static void WriteMultiReg(const char *cmd);
...
/*******************************************************************************
* Function Name  : lis2de12_AppTick
...
static void StateMachine(void)
{
  ...
  case RM:
    if(zoffFlag) {
      ReadMultiReg("MULTI-R");
      state = stateOld;
    }
    break;
  case WM:
    if(zoffFlag) {
      WriteMultiReg("MULTI-W");
      state = stateOld;
    }
    break;
  case DEBUG:
...
static void Start(void)
{
  ...
  if(ValBit(dataReady, DATAREADY_BIT)) {
    ...
    buffer[3]   = SPI_Mems_Read_Reg( OUT_Y_H );	//reads Y MSB
    buffer[4]   = SPI_Mems_Read_Reg( OUT_Z_H );	//reads Z MSB
...
static void ReadMultiReg(const char *cmd)
{
  ...
  while (*addrp != '\0') {
    ...
    //read_dev_reg(&data, address, lis2dg_io_mode);
    data = SPI_Mems_Read_Reg(address);
    CharToString(data, response);
...
static void WriteMultiReg(const char *cmd)
{
  ...
  while (*addrp != '\0') {
    if( isxdigit(addrp[0]) && isxdigit(addrp[1]) &&
        isxdigit(addrp[2]) && isxdigit(addrp[3]) ) {

      address = StringToChar(addrp);
      data = StringToChar(addrp + 2);
      //write_dev_reg(data, address, lis2dg_io_mode);
      SPI_Mems_Write_Reg(address, data);
...
static void Debug(void)
{
  ...
  if(ValBit(dataReady, DATAREADY_BIT)) {
    ...
    data = (int8_t) SPI_Mems_Read_Reg(OUT_X_H);
    sprintf((char*)buffer,"X=%+05d ",data);

    data = (int8_t) SPI_Mems_Read_Reg(OUT_Y_H);
    sprintf((char*)&buffer[8],"Y=%+05d ",data);

    data = (int8_t) SPI_Mems_Read_Reg(OUT_Z_H);
