...
void drawenglish_pad(WM_HWIN hWin)
{
    ...
    {
    	//创建按钮
        keypad_dev.EngButton_Handle[i] = BUTTON_CreateEx(_aEngButtonData[i].xPos,
            _aEngButtonData[i].yPos,
            _aEngButtonData[i].xSize,
            _aEngButtonData[i].ySize,
            hWin,
...
void drawnumber_pad(WM_HWIN hWin)
{
    ...
    {
    	//创建按钮
        keypad_dev.NumButton_Handle[i] = BUTTON_CreateEx(_aNumButtonData[i].xPos,
            _aNumButtonData[i].yPos,
            _aNumButtonData[i].xSize,
            _aNumButtonData[i].ySize,
            hWin,
            WM_CF_SHOW | WM_CF_HASTRANS,
            0,
            ID_BUTTON + i);
...
void drawsign_pad(WM_HWIN hWin)
{
    ...
    {
    	//创建按钮
        keypad_dev.SignButton_Handle[i] = BUTTON_CreateEx(_aSinButtonData[keypad_dev.signpad_flag][i].xPos,
            _aSinButtonData[keypad_dev.signpad_flag][i].yPos,
            _aSinButtonData[keypad_dev.signpad_flag][i].xSize,
            _aSinButtonData[keypad_dev.signpad_flag][i].ySize,
            hWin,
            WM_CF_SHOW | WM_CF_HASTRANS,
            0,
            ID_BUTTON + i);
...
void drawchinese1_pad(WM_HWIN hWin)
{
    ...
    {
    	//创建按钮
        keypad_dev.ChineseButton_Handle[i] = BUTTON_CreateEx(_aChineseButtonData1[i].xPos,
            _aChineseButtonData1[i].yPos,
            _aChineseButtonData1[i].xSize,
            _aChineseButtonData1[i].ySize,
            hWin,
            WM_CF_SHOW | WM_CF_HASTRANS,
            0,
            ID_BUTTON + i);
...
void drawchinese2_pad(WM_HWIN hWin)
{
    ...
    {
    	//创建按钮
        keypad_dev.ChineseButton_Handle[i + 12] = BUTTON_CreateEx(_aChineseButtonData2[i].xPos,
            _aChineseButtonData2[i].yPos,
            _aChineseButtonData2[i].xSize,
            _aChineseButtonData2[i].ySize,
            hWin,
            WM_CF_SHOW | WM_CF_HASTRANS,
            0,
            ID_BUTTON + i+12);
...
//                BUTTON_SetTextColor(_aahButtonOk, BUTTON_CI_UNPRESSED, GUI_BLACK);
    ...
    case SYSSET_VALUE:
        ...
        break;
    case CONSET_VALUE:
        if (htmpChild == _hWinManagerConSet)
        {
            pCon = CONGetHandle(0);
        }
        else
        ...
        case 21:
           ...
           // pCon->info.ucSocketType = 0;
            ...
            cfg_set_uint8(pathEVSECfg, &result_input[0], "%s:%d.%s", jnCONArray, pCon->info.ucCONID, jnSocketType);
