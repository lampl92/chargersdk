...
void create_bmppicture(uint8_t *filename,int x0,int y0,int Xsize,int Ysize)
{
    ...
    fdes.buff = (char *)malloc(800*1024);
    memset(fdes.buff, 0, 800 * 1024);
    fdes.size = 0;
    //创建一个文件，路径为filename,如果文件原本就存在的话会被新建的文件覆盖掉！
    //	GUI_BMP_SerializeEx(_WriteByte2File,x0,y0,Xsize,Ysize,&hFile);
    extern void myGUI_BMP_Serialize(GUI_CALLBACK_VOID_U8_P * pfSerialize, void * p);
    myGUI_BMP_Serialize(_WriteByte2File, &fdes);
    fdes.fd = yaffs_open(filename, O_CREAT | O_TRUNC | O_RDWR, S_IREAD | S_IWRITE);
    nWritten = yaffs_write(fdes.fd, fdes.buff, fdes.size);
    if (nWritten != fdes.size)
    {
        printf_safe("size:%d\n", fdes.size);
    }
	yaffs_close(fdes.fd);	//关闭文件
    free(fdes.buff);
}
