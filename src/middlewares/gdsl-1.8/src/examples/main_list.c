...
affiche_liste_chaines_fwd (gdsl_list_t l)
{
    ...
    printf_safe ("%s (->) = ( ", gdsl_list_get_name (l));
...
int main_list (void)
{
    ...
    do
	{
	    printf_safe ("\t\tMENU - LIST\n\n");
	    printf_safe ("\t 1> Create a cell\n");
	    printf_safe ("\t 2> Remove the first cell\n");
	    printf_safe ("\t 3> Remove the last cell\n");
	    printf_safe ("\t 4> Remove a cell\n");
	    printf_safe ("\t 5> Display list in forward order\n");
	    printf_safe ("\t 6> Display list in backward order\n");
	    printf_safe ("\t 7> Flush list\n");
	    printf_safe ("\t 8> Size of list\n");
	    printf_safe ("\t 9> Dump list\n");
	    printf_safe ("\t10> XML dump of list\n");
	    printf_safe ("\t11> Search for a place\n");
	    printf_safe ("\t12> Search for an element\n");
	    printf_safe ("\t13> Sort of list\n");
	    printf_safe ("\t14> Greatest element of list\n");
	    printf_safe ("\t 0> Quit\n\n");
	    printf_safe ("\t\tYour choice: ");
		...
		case 1:
			...
			do
				...
				printf_safe ("\t\tMENU - CELL INSERTION\n\n");
				printf_safe ("\t1> Insert cell at the beginning of the list\n");
				printf_safe ("\t2> Insert cell at end of list\n");
				printf_safe ("\t3> Insert cell after another cell\n");
				printf_safe ("\t4> Insert cell before another cell\n");
				printf_safe ("\t5> Display the list\n");
				printf_safe ("\t0> RETURN TO MAIN MENU\n\n");
				printf_safe ("\t\tYour choice: ");
				//scanf ("%d", &choix );
				xgets(&choix, 1);
				...
				switch (choix)
					...
					if (gdsl_list_is_empty (l))
						...
						printf_safe ("Name of cell after which you want to insert: ");
						//scanf ("%s", Nom);
						xgets(&choix, 1);

						if (!gdsl_list_cursor_move_to_value (c, compare_strings, Nom))
						    {
							printf_safe ("The cell '%s' doesn't exist\n", Nom);
						    }
						else
						    {
					...
					if (gdsl_list_is_empty (l))
						...
						printf_safe ("Name of cell before which you want to insert: ");
						//scanf ("%s", Nom);
						xgets(&choix, 1);
		...
		case 4:
			...
			if (gdsl_list_is_empty (l))
			    ...
			    {
				printf_safe ("Name of cell to remove: ");
				//scanf ("%s", nom);
				xgets(&choix, 1);
				...
				if (!gdsl_list_delete (l, compare_strings, nom))
				    ...
				    {
					printf_safe ("The cell '%s' is removed from list\n", nom);
				    }
			    }
		    }
		    break;
		...
		case 11:
			...
			printf_safe ("Enter the position of the place to search for: ");
			//scanf ("%d", & pos);
			xgets(&pos, 1);
		...
		case 12:
			...
			printf_safe ("Name of cell to search for: ");
			scanf ("%s", nom);
			...
			if (e == NULL)
			    ...
			    {
				printf_safe ("The cell '%s' was found: ", nom);
				print_string (e, stdout, GDSL_LOCATION_UNDEF, NULL);
				printf_safe ("\n");
			    }
		    }
