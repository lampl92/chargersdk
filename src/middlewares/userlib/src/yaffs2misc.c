...
int check_pattern_file(char *fn)
{
	...
	for(i = 0; i < size; i+=256)
	{
		...
		if(!ok)
		{
		   printf_safe("pattern check failed on file %s, size %d at position %d. Got %x instead of %x\n",
					fn,size,i,marker,~i);
		}
	}
	yaffs_close(h);
...
void create_file_of_size(const char *fn,int syze)
{
	...
	while (syze > 0)
	{
		...
		if(result != n){
			printf_safe("Wrote %d, should have been %d. syze is %d\n",result,n,syze);
			syze = 0;
		} else
			syze-=n;
		iteration++;
...
void verify_file_of_size(const char *fn,int syze)
{
	...
	while (iterations > 0)
	{
		...
		if (result)
			printf_safe("result in line %d is %d", __LINE__, result);
		yy[l] = 0;

		if(strcmp(xx,yy)){
			printf_safe("=====>>>>> verification of file %s failed near position %lld\n",fn,(long long)yaffs_lseek(h,0,SEEK_CUR));
		}
		iterations--;
	}
	yaffs_close (h);
...
void fill_disk(const char *path,int nfiles)
{
	...
	for(n = 0; n < nfiles; n++)
	{
		...
		printf_safe("writing file %s handle %d ",str, h);

		while ((result = yaffs_write(h,xx,600)) == 600)
		{
			f = yaffs_freespace(path);
		}
		printf_safe("last freespace was %d\n", f);
...
void fill_disk_and_delete(const char *path, int nfiles, int ncycles)
{
	...
	for(i = 0; i < ncycles; i++)
	{
		...
		for(j = 0; j < nfiles; j++)
		{
			...
			printf_safe("unlinking file %s, result %d\n", str, result);
		}
	}
}
...
void leave_unlinked_file(const char *path,int maxIterations,int siz)
{
	...
	printf_safe("Leaving file %s open\n",str);
...
void dumpDirFollow(const char *dname)
{
	...
	else
	{
		while((de = yaffs_readdir(d)) != NULL)
		{
			...
			printf_safe("%s ino %d length %d mode %X ",
				de->d_name, (int)s.st_ino, (int)s.st_size, s.st_mode);
			switch(s.st_mode & S_IFMT)
			{
				case S_IFREG: printf_safe("data file"); break;
				case S_IFDIR: printf_safe("directory"); break;
				case S_IFLNK: printf_safe("symlink -->");
							  if(yaffs_readlink(str,str,100) < 0)
								printf_safe("no alias");
							  else
								printf_safe("\"%s\"",str);
							  break;
				default: printf_safe("unknown"); break;
			}
	...
	printf_safe("Free space in %s is %d\n\n",dname,(int)yaffs_freespace(dname));
...
void dump_directory_tree_worker(const char *dname,int recursive)
{
	...
	else
	{
		while((de = yaffs_readdir(d)) != NULL)
		{
			...
			printf_safe("%s inode %d length %d mode %X ",
...
void dump_directory_tree_worker_fd(const char *dname,int recursive)
{
	...
	if(h < 0)
	{
		printf_safe("open of dir failed\n");
	}
	else
	{
		printf_safe("using fd %d\n", h);
...
static void PermissionsCheck(const char *path, mode_t tmode, int tflags,int expectedResult)
{
	...
	if(yaffs_chmod(path,tmode)< 0) printf_safe("chmod failed\n");

	fd = yaffs_open(path,tflags,0);

	if((fd >= 0) != (expectedResult > 0))
	{
		printf_safe("Permissions check %x %x %d failed\n",tmode,tflags,expectedResult);
	}
	else
	{
		printf_safe("Permissions check %x %x %d OK\n",tmode,tflags,expectedResult);
	}
...
int long_test(int argc, char *argv[])
{
	...
	printf_safe("open /boot/b1 readonly, f=%d\n",f);

	f = yaffs_open("/boot/b1", O_CREAT,S_IREAD | S_IWRITE);

	printf_safe("open /boot/b1 O_CREAT, f=%d\n",f);


	r = yaffs_write(f,"hello",1);
	printf_safe("write %d attempted to write to a read-only file\n",r);
	...
	printf_safe("open /boot/b1 O_RDWR,f=%d\n",f);


	r = yaffs_write(f,"hello",2);
	printf_safe("write %d attempted to write to a writeable file\n",r);
	r = yaffs_write(f,"world",3);
	printf_safe("write %d attempted to write to a writeable file\n",r);

	r= yaffs_lseek(f,0,SEEK_END);
	printf_safe("seek end %d\n",r);
	memset(buffer,0,20);
	r = yaffs_read(f,buffer,10);
	printf_safe("read %d \"%s\"\n",r,buffer);
	r= yaffs_lseek(f,0,SEEK_SET);
	printf_safe("seek set %d\n",r);
	memset(buffer,0,20);
	r = yaffs_read(f,buffer,10);
	printf_safe("read %d \"%s\"\n",r,buffer);
	memset(buffer,0,20);
	...
	r= yaffs_lseek(f,0,SEEK_END);
	r = yaffs_read(f,buffer,10);
	printf_safe("read at end returned  %d\n",r);
	r= yaffs_lseek(f,500,SEEK_END);
	r = yaffs_read(f,buffer,10);
	printf_safe("read past end returned  %d\n",r);
	...
	printf_safe("symlink alias is %s\n",str);
...
int root_perm_remount(const char *path)
{
   ...
   yaffs_lstat(path,&s);
   printf_safe("root perms after mount %x\n",s.st_mode);

   yaffs_chmod(path, 0777);

   yaffs_lstat(path,&s);
   printf_safe("root perms after setting to 0777 is  %x\n",s.st_mode);
...
int directory_rename_test(void)
{
	...
	printf_safe("Do rename (should fail)\n");
	...
	printf_safe("Do rename (should not fail)\n");
...
int cache_bypass_bug_test(void)
{
	...
	printf_safe("buffer 1 %s\n",buffer1);
	printf_safe("buffer 2 %s\n",buffer2);

	if(strncmp(buffer1,buffer2,20))
	{
		printf_safe("Cache bypass bug detected!!!!!\n");
	}
...
int free_space_check(void)
{
	...
	printf_safe("%d free when disk full\n",f);
	return 1;
}
...
int truncate_test(void)
{
	...
	printf_safe("truncated length is %d\n",l);
	...
	printf_safe("read %d bytes:", r);

	for(i = 0; i < r; i++) printf_safe("[%02X]",y[i]);
...
void long_name_test(const char *mountpt)
{
	...
	for(i = 0; i < 1; i++)
	{
		...
		printf_safe("Files at start\n");
		dumpDir(mountpt);

		printf_safe("Creating file %s\n",fullName);

		f = yaffs_open(fullName,O_CREAT | O_RDWR,0);
		yaffs_close(f);

		printf_safe("Result %d\n",f);

		printf_safe("Files\n");
		dumpDir(mountpt);

		printf_safe("Deleting %s\n",fullName);
...
void lookup_test(const char *mountpt)
{
	...
	else
	{
		...
		{
			printf_safe("unlinking %s\n",de->d_name);
			yaffs_unlink(de->d_name);
		}

		printf_safe("%d files deleted\n",i);
	}
...
void link_test0(const char *mountpt)
{
	...
	yaffs_unlink(namea);
	printf_safe("a unlinked\n");
        dumpDir(mountpt);

	yaffs_unlink(nameb);
	printf_safe("b unlinked\n");
        dumpDir(mountpt);

	result = yaffs_open(namea,O_CREAT| O_RDWR,0666);
        yaffs_close(result);
	printf_safe("a created\n");
        dumpDir(mountpt);
...
void handle_test(const char *mountpt)
{
	...
	yaffs_mount(mountpt);
        ...
        printf_safe("Start cycle %d\n",cycle);
 	i = 0;
	do {
        h = yaffs_open(a, O_CREAT | O_TRUNC | O_RDWR, S_IREAD | S_IWRITE);
	printf_safe("%d  handle %d\n",i,h);
	i++;
	} while(h >= 0);
...
void simple_rw_test(const char *mountpt)
{
	...
	for(i = 100000; i < 200000; i++){
		...
		if(result != 4 || x != i){
			printf_safe("read error %d %x %x\n",i,result,x);
		}
	}

	printf_safe("Simple rw test passed\n");
...
void write_200k_file(const char *fn, const char *fdel, const char *fdel1)
{
   ...
   if( offs != 100000)
   {
   	printf_safe("Could not write file\n");
   }
...
void verify_200k_file(const char *fn)
{
   ...
   for(i = 0; i < 200000 && errCount < 10; i+= 10)
   {
	...
	if(strncmp(x,s,10) != 0)
	{
		printf_safe("File %s verification failed at %d\n",fn,i);
		errCount++;
	}
   }
   if(errCount >= 10)
   	printf_safe("Too many errors... aborted\n");
...
void checkpoint_upgrade_test(const char *mountpt, int nmounts)
{
	...
	printf_safe("Create start condition\n");
	yaffs_start_up();
	yaffs_mount(mountpt);
	yaffs_mkdir(a,0);
	sprintf(b,"%s/zz",a);
	...
	printf_safe("Umount/mount attempt full\n");
	yaffs_unmount(mountpt);

	yaffs_mount(mountpt);

	printf_safe("unlink small file\n");
	yaffs_unlink(c);
	dump_directory_tree(mountpt);

	printf_safe("Umount/mount attempt\n");
	yaffs_unmount(mountpt);
	yaffs_mount(mountpt);
...
void huge_array_test(const char *mountpt,int n)
{
	...
	while(n>0){
		...
		while((space = yaffs_freespace(mountpt)) > 25000000){
			...
			fnum++;
			printf_safe("create file %s, free space %d\n",a,space);
			create_file_of_size(a,10000000);
			printf_safe("verifying file %s\n",a);
...
void random_small_file_test(const char *mountpt,int iterations)
{
	...
	for(n = 0; n < iterations; n++){
		...
		for(i = 0; i < NSMALLFILES; i++) {
			...
			if(h[i] < -1)
				printf_safe("Could not open yaffs file %d %d error %d\n",n,i,h[i]);
			else {
				r = r & 7;
				switch(r){
					case 0:
...
static void list_xattr(const char *path)
{
	...
	printf_safe("xattribs for %s, result is %d\n",path,list_len);
	while(n < list_len){
		len = strlen(list + n);
		printf_safe("\"%s\" value ",list+n);
...
void basic_utime_test(const char *mountpt)
{
	...
	yaffs_fstat(h,&st);
	printf_safe(" times %lu %lu %lu\n",
			st.yst_atime, st.yst_ctime, st.yst_mtime);

	utb.actime = 1000;
	utb.modtime = 2000;
	result = yaffs_futime(h,&utb);
	printf_safe("futime to a 1000 m 2000 result %d\n",result);
	yaffs_fstat(h,&st);
	printf_safe(" times %lu %lu %lu\n",
			st.yst_atime, st.yst_ctime, st.yst_mtime);
	...
	printf_safe("utime to a 5000 m 8000 result %d\n",result);
	yaffs_fstat(h,&st);
	printf_safe(" times %lu %lu %lu\n",
			st.yst_atime, st.yst_ctime, st.yst_mtime);

	result = yaffs_utime(name, NULL);
	printf_safe("utime to NULL result %d\n",result);
	yaffs_fstat(h,&st);
	printf_safe(" times %lu %lu %lu\n",
			st.yst_atime, st.yst_ctime, st.yst_mtime);
...
void basic_xattr_test(const char *mountpt)
{
	...
	printf_safe("Add an attribute\n");
	val1 = 0x123456;
	result = yaffs_setxattr(name,"foo",&val1,sizeof(val1),0);
	printf_safe("wrote attribute foo: result %d\n",result);
	list_xattr(name);
	printf_safe("Add an attribute\n");
	val1 = 0x7890;
	result = yaffs_setxattr(name,"bar",&val1,sizeof(val1),0);
	printf_safe("wrote attribute bar: result %d\n",result);
	list_xattr(name);

	printf_safe("Get non-existanrt attribute\n");
	print_xattrib_val(name,"not here");

	printf_safe("Delete non existing attribute\n");
	yaffs_removexattr(name,"not here");
	list_xattr(name);

	printf_safe("Remove foo\n");
	yaffs_removexattr(name,"foo");
	list_xattr(name);

	printf_safe("Remove bar\n");
...
void big_xattr_test(const char *mountpt)
{
	...
	printf_safe("Add a large  attribute\n");
	memset(val,0x1,sizeof(val));
	result = yaffs_setxattr(name,"aaa",val,200,0);
	printf_safe("wrote attribute aaa: result %d\n",result);
	list_xattr(name);

	printf_safe("Add a large  attribute\n");
	memset(val,0x2,sizeof(val));
	result = yaffs_setxattr(name,"bbb",val,1000,0);
	printf_safe("wrote attribute bbb: result %d\n",result);
	list_xattr(name);

	printf_safe("Replace attribute\n");
...
{
	printf_safe("%s\n",str);
	printf_safe( "space free %d erased %d "
...
void link_follow_test(const char *mountpt)
{
	...
	if (result < 0)
		printf_safe("At line %d result is %d\n", __LINE__, result);
...
void max_files_test(const char *mountpt)
{
	...
	for(i = 0; i < 5000; i++) {
		...
		h = yaffs_open(fn,O_CREAT| O_RDWR, S_IREAD | S_IWRITE);
		if(h < 0)
			printf_safe("File %s not created\n", fn);
		yaffs_write(h,fn,100);
		result = yaffs_close(h);
	}
	for(i = 0; i < 5; i++){
...
void case_insensitive_test(const char *mountpt)
{
	...
	while(*x) {
		*x = toupper(*x);
		...
		printf_safe("wrong file read\n");
        } else {
		printf_safe("File %s is the same as file %s\n", fn, fn2);
        }

        ret = yaffs_stat(fn2, &s);

	printf_safe("renaming\n");
...
{
         printf_safe("About to do first yaffs_start\n");
         yaffs_start_up();
         printf_safe("started\n");
         printf_safe("First mount returns %d\n", yaffs_mount(mountpt));
         printf_safe("About to do second yaffs_start\n");
         yaffs_start_up();
         printf_safe("started\n");
         printf_safe("Second mount returns %d\n", yaffs_mount(mountpt));
}
...
void write_big_sparse_file(int h)
{
	...
	for(i = 0; i < N_WRITES; i++) {
		printf_safe("writing at %d\n", (int)offset);
		set_buffer(i);
		pos = yaffs_lseek(h, offset, SEEK_SET);
		if(pos != offset) {
			printf_safe("mismatched seek pos %d offset %d\n",
				(int)pos, (int)offset);
			perror("lseek64");
			exit(1);
		}
		...
		if(wrote != n) {
			printf_safe("mismatched write wrote %d n %d\n", wrote, n);
			exit(1);
		}
...
void verify_big_sparse_file(int h)
{
	...
	for(i = 0; i < N_WRITES * STRIDE; i++) {
		...
		if(result != n) {
			printf_safe("mismatched read result %d n %d\n", result, n);
			exit(1);
		}
		...
		if(memcmp(xxbuffer, check_buffer, n)) {
			...
			printf_safe("buffer at %d mismatches\n", (int)pos);
			printf_safe("xxbuffer ");
			for(j = 0; j < 20; j++)
				printf_safe(" %d",xxbuffer[j]);
			printf_safe("\n");
			printf_safe("check_buffer ");
			for(j = 0; j < 20; j++)
				printf_safe(" %d",check_buffer[j]);
			printf_safe("\n");
	...
	printf_safe("%d checks passed, %d checks failed\n", checks_passed, checks_failed);
...
void large_file_test(const char *mountpt)
{
	...
	printf_safe("Job done\n");
	yaffs_unmount(mountpt);

	yaffs_mount(mountpt);
	printf_safe("mounted again\n");
        dumpDir(mountpt);
	handle = yaffs_open(fullname, O_RDONLY, 0);
	verify_big_sparse_file(handle);
	yaffs_unmount(mountpt);
	...
	printf_safe("mounted with no checkpt\n");
        dumpDir(mountpt);
	handle = yaffs_open(fullname, O_RDONLY, 0);
	verify_big_sparse_file(handle);
	yaffs_unmount(mountpt);
...
void xx_test(const char *mountpt)
{
	...
	printf_safe("unmount and remount\n");
...
void format_test(const char *mountpt)
{
	...
	printf_safe("yaffs_format(...,0, 0, 0) of unmounted returned %d."
		" Should return 0\n\n\n", ret);

	yaffs_mount(mountpt);

	ret = yaffs_format(mountpt, 0, 0, 0);
	printf_safe("yaffs_format(...,0, 0, 0) of mounted returned %d."
		" Should return -1 (busy)\n\n\n", ret);

	ret = yaffs_format(mountpt, 1, 0, 0);
	printf_safe("yaffs_format(...,1, 0, 0) of mounted returned %d."
		" Should return 0.\n\n\n", ret);

	ret = yaffs_mount(mountpt);
	printf_safe("mount should return 0 returned %d\n\n\n", ret);

	ret = yaffs_format(mountpt, 1, 0, 1);
	printf_safe("yaffs_format(...,1, 0, 1) of mounted returned %d."
		" Should return 0.\n\n\n", ret);

	ret = yaffs_mount(mountpt);
	printf_safe("mount should return -1 returned %d\n", ret);
}
...
void dir_rename_test(const char *mountpt)
{
         ...
         printf_safe("Try to rename %s to %s\n", fname, dname);
         ret = yaffs_rename(fname, dname);
         printf_safe("result %d, %d\n", ret, yaffs_get_error());
	...
	for(i = 1; i < 2000; i++) {
		...
		while (fsize > 0) {
			...
			if (wrote != sizeof(buffer)) {
				printf_safe("Writing file %s, only wrote %d bytes\n", fn, wrote);
				break;
			}
			fsize -= wrote;
		}
...
int yaffs2test_main(int argc, char *argv[])
{
	(void) argc;
	(void) argv;
