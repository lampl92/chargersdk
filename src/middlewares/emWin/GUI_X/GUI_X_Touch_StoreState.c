...
void TouchTask(void) {
  ...
  do 
  {
      Pressed = PEN?0:1;// TBD: Insert function which returns:
    ...
    if (Pressed) 
    {
      x = TP_Read_XOY(CMD_RDX);// TBD: Insert function which reads current x value
      y = TP_Read_XOY(CMD_RDY);// TBD: Insert function which reads current y value
      //
