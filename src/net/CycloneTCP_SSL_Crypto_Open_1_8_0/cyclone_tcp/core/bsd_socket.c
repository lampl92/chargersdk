   ...
   const void *optval, socklen_t optlen)
{
   ...
   if(optval != NULL)
   {
      ...
      if(level == SOL_SOCKET)
      {
         ...
         case SO_RCVTIMEO:
            //Check the length of the option
            if(optlen >= (socklen_t) sizeof(timeval_cyclone))
            {
               //Cast the option value to the relevant type
               t = (timeval_cyclone *) optval;
 ...
 *   are ready and contained in the fd_set_t structures, zero if the time
 *   limit expired, or SOCKET_ERROR if an error occurred
 **/

int_t select_cyclone(int_t nfds, fd_set_cyclone *readfds, fd_set_cyclone *writefds,
   fd_set_cyclone *exceptfds, const timeval_cyclone *timeout)
{
   int_t i;
   int_t j;
   int_t n;
...
void selectFdZero(fd_set_cyclone *fds)
{
...
void selectFdSet(fd_set_cyclone *fds, int_t s)
{
   int_t i;
...
void selectFdClr(fd_set_cyclone *fds, int_t s)
{
   int_t i;
   int_t j;
...
int_t selectFdIsSet(fd_set_cyclone *fds, int_t s)
{
   int_t i;
